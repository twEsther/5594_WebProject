...
/* .input[name='addlist']{width: 100px;} */
		...
		<h4 class="leftBar_listheader">播放清單</h4>
			<div class="leftBar_listcontent">	
				...
				<div class="leftBar_heartlist">
					<a href="<%= request.getContextPath() %>/member/music_love.jsp"><button type="submit" class="heart">
...
<% 
	...
	//List<Music> list = new ... (200902解惑)
	
	String lyricslanguage = (String)request.getParameter("lyricslanguage");
...
%>
		...
		<div id="root_mainView" class="root_mainView" style="background-color:##d8d8d7;">
			...
			<div class="info">
				<div class="infoimg">
					<%	if("中文".equals(lyricslanguage)){ %> 
						<img class="infoIMG" src="<%= request.getContextPath() %>/images/chinese-300.png"/>
					<% } else if ("英文".equals(lyricslanguage)) { %>
						<img class="infoIMG" src="<%= request.getContextPath() %>/images/WEST-300.png"/>				
					<% } else if ("日韓".equals(lyricslanguage)) { %>
						<img class="infoIMG" src="<%= request.getContextPath() %>/images/JPKR-300.png"/>
					<% } else { %>
						<img class="infoIMG" src="<%= request.getContextPath() %>/images/allmusic-300.png"/>
					<% } %>
				</div>
					<!-- ★★★想一想：lyricslanguage.equals("日韓") 這樣會nullPointException!!!看網址	-->
					
				<div class="listname">
					<h6>播放清單</h6>
					<%	if ("中文".equals(lyricslanguage)){ %>	
						<div class="list_name">華語榜</div>
						<div class="description">中文歌手、中文歌手、中文歌手 還有更多</div>
					<% } else if ("英文".equals(lyricslanguage)) { %>
						<div class="list_name">西洋榜</div>
						<div class="description">英文歌手、英文歌手、英文歌手 還有更多</div>
					<% } else if ("日韓".equals(lyricslanguage)) { %>
						<div class="list_name">日韓榜</div>
						<div class="description">日韓歌手、日韓歌手 還有更多</div>
					<% } else { %>
						<div class="list_name">所有歌曲</div>
						<div class="description">中文歌手、英文歌手、日韓歌手 還有更多</div> 
					<% } %>
				</div>
			</div>
...
<%
	...
	} else if(lyricslanguage!=null&&lyricslanguage.length()>0){
		list = service.getAllMusicByLanguage(lyricslanguage);
	} 
	else if (love!=null && love.length()>0) {
		list = service.getMemberLoveList(member.getEmail(),love);
		list = service.getMemberPlayList(member.getEmail(),love);
	}
//	else if (love!=null && love.length()>0) {
//		list = service.getMemberPlayList(member.getEmail(),love);
		//out.print(songList);
...
%>
			...
			<form action="add_list.do" id="addlist" method="POST"> <!-- 雖然要傳的東西有兩種，但就在同一個Servlet寫兩個商業邏輯 -->
				<table id="mainplaylist" style='border-bottom:1px solid #bababa'>
					<tbody>
						<tr class="playlist_container">
							...
							<td class="smallpic"><div id="video">
							<iframe 
								width="145" height="82" src="<%= music.getYturl() %>"
								frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture">
							</iframe></div></td>	
							<td class="title"><span><%= music.getArtist()%> - <%= music.getSong() %></span></td>
							<td class="time"><%= music.getDuration() %></td>
							<td class="addtolist">	
								<select id="addlist" name="love" style="width:80px;"> <!-- onchange="javascript:location.href=this.value;" -->
										<option selected>加到歌單</option>
								      <!-- <option value="<%= request.getContextPath() %>/member/music_list.jsp?love=最近常聽">最近常聽</option> -->	
							   			<option value="最近常聽" >最近常聽</option>
							   			<option value="情歌精選" >情歌精選</option>
							   			<option value="西洋派對" >西洋派對</option>
							   	</select>  	
							</td>							   			  			
							<td class="share"><input type="submit" name="love" value="按讚"></td> <!-- value是飯.是便當 -->
...
<script>
	...
	player = new YT.Player('video', {
	    videoId: '2HwQQwDWO2s', // Providing the video ID.
	    height: '82',
	    width: '145',
	    playerVars:{
	    	autoplay:0,
	    ...
	    'onStateChange': onStateChange, // Initializing the on change action function.
	    }
	});
	}
	...
	function onStateChange(event){
	    ...
	    if(event.data == YT.PlayerState.PAUSE){
	    	$("#paly").click(playHandler);
	    }
	    
		//video=$("#video")[0];
    	$("#video").on("play", playHandler);
    	$("#video").on("pause", pauseHandler);
	}
