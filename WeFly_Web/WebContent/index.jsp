...
<!-- 要從html改成jsp之前一定要先在上面加入編碼，否則會亂碼 -->
<!-- 加完編碼後要記得存檔，就去改副檔名 -->
<!-- 這時原本的html會全部變黑色的因為看不懂，這時候只要再重開就好了 -->
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<%-- 	$(function(){
		...
		$("#chinese").click(function(e){
			e.preventDefault();
			$(".language").addClass(".rank");
			$(".language:contains('<%= music.getLyricslanguage() %>')").removeClass(".rank");
		})
	)}; --%>
...
<body>		
		<div class="root_leftBar" style="background-color:#646462;"> <!-- #51514f -->
				...
				<div>
					<ul class="mainView">
						<li class="mylist1_heart">
							<a href="<%= request.getContextPath() %>/member/music_love.jsp"><button type="submit" class="heartlist">
					...
					<ul class="all_lists">
						<li class="mylist">
							<a id="est" class="rank" href="<%= request.getContextPath() %>/member/recently_listen.jsp">
							<button type="button" class="list1">
						...
						<li class="mylist">
							<a id="lovesong" class="rank" href="<%= request.getContextPath() %>/member/love_song.jsp">
							<button type="button" class="list1">
						...
						<li class="mylist">
							<a id="west" class="rank" href="<%= request.getContextPath() %>/member/west_party.jsp">
							<button type="button" class="list1">
